...
from get_attn_maps import compute_token_sims, plot_attn_vis
...
def main(args):
    ...
    for idx, query in enumerate([args.query_1, args.query_2]):
        ...
        EXPS[idx] = EXPS[idx] + "_" + str(idx)
        
        os.makedirs(f"/data/data0/akane/attn_tensors/", exist_ok=True)
        torch.save(hook_cache, f"/data/data0/akane/attn_tensors/{MODEL}_{EXPS[idx]}.pt")
        torch.save(ModalityBuffer.inputs_emb_modalities, f"/data/data0/akane/attn_tensors/{MODEL}_{EXPS[idx]}_modalities.pt")
